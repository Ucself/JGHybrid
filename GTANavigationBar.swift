//
//  GTANavigationBar.swift
//  iOSJade
//
//  Created by goingta on 2018/12/26.
...
extension UINavigationBar:WRAwakeProtocol
{
    ...
    fileprivate func wr_setBackgroundImage(image:UIImage)
    {
        ...
        if (backgroundImageView == nil)
        {
            ...
            backgroundImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: GTANavigationBar.navBarBottom()))
    ...
    fileprivate func wr_setBackgroundColor(color:UIColor)
    {
        ...
        if (backgroundView == nil)
        {
            // add a image(nil color) to _UIBarBackground make it clear
            setBackgroundImage(UIImage(), for: .default)
            backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: GTANavigationBar.navBarBottom()))
...
extension UINavigationController: WRFatherAwakeProtocol
{
    override open var preferredStatusBarStyle: UIStatusBarStyle {
        return topViewController?.statusBarStyle ?? GTANavigationBar.defaultStatusBarStyle
    }
    ...
    fileprivate func updateNavigationBar(fromVC: UIViewController?, toVC: UIViewController?, progress: CGFloat)
    {
        ...
        let fromBarTintColor = fromVC?.navBarBarTintColor ?? GTANavigationBar.defaultNavBarBarTintColor
        let toBarTintColor   = toVC?.navBarBarTintColor ?? GTANavigationBar.defaultNavBarBarTintColor
        let newBarTintColor  = GTANavigationBar.middleColor(fromColor: fromBarTintColor, toColor: toBarTintColor, percent: progress)
        setNeedsNavigationBarUpdate(barTintColor: newBarTintColor)
        
        // change navBarTintColor
        let fromTintColor = fromVC?.navBarTintColor ?? GTANavigationBar.defaultNavBarTintColor
        let toTintColor = toVC?.navBarTintColor ?? GTANavigationBar.defaultNavBarTintColor
        let newTintColor = GTANavigationBar.middleColor(fromColor: fromTintColor, toColor: toTintColor, percent: progress)
        setNeedsNavigationBarUpdate(tintColor: newTintColor)
        
        // change navBarTitleColor
        //        let fromTitleColor = fromVC?.navBarTitleColor ?? GTANavigationBar.defaultNavBarTitleColor
        //        let toTitleColor = toVC?.navBarTitleColor ?? GTANavigationBar.defaultNavBarTitleColor
        //        let newTitleColor = GTANavigationBar.middleColor(fromColor: fromTitleColor, toColor: toTitleColor, percent: progress)
        //        setNeedsNavigationBarUpdate(titleColor: newTitleColor)
        
        // change navBar _UIBarBackground alpha
        let fromBarBackgroundAlpha = fromVC?.navBarBackgroundAlpha ?? GTANavigationBar.defaultBackgroundAlpha
        let toBarBackgroundAlpha = toVC?.navBarBackgroundAlpha ?? GTANavigationBar.defaultBackgroundAlpha
        let newBarBackgroundAlpha = GTANavigationBar.middleAlpha(fromAlpha: fromBarBackgroundAlpha, toAlpha: toBarBackgroundAlpha, percent: progress)
    ...
    @objc public func wr_popToRootViewControllerAnimated(_ animated: Bool) -> [UIViewController]?
    {
        var displayLink:CADisplayLink? = CADisplayLink(target: self, selector: #selector(popNeedDisplay))
        displayLink?.add(to: RunLoop.main, forMode: .commonModes)
        CATransaction.setCompletionBlock {
    ...
    @objc public func wr_pushViewController(_ viewController: UIViewController, animated: Bool)
    {
...
extension UINavigationController: UINavigationBarDelegate
{
    ...
    private func dealInteractionChanges(_ context: UIViewControllerTransitionCoordinatorContext)
    {
        let animations: (UITransitionContextViewControllerKey) -> () = {
            let curColor = context.viewController(forKey: $0)?.navBarBarTintColor ?? GTANavigationBar.defaultNavBarBarTintColor
            let curAlpha = context.viewController(forKey: $0)?.navBarBackgroundAlpha ?? GTANavigationBar.defaultBackgroundAlpha
...
extension UIViewController: WRAwakeProtocol
{
    fileprivate struct AssociatedKeys
    {
        ...
        static var navBarBarTintColor: UIColor = GTANavigationBar.defaultNavBarBarTintColor
        static var navBarBackgroundAlpha:CGFloat = 1.0
        static var navBarTintColor: UIColor = GTANavigationBar.defaultNavBarTintColor
        static var navBarTitleColor: UIColor = GTANavigationBar.defaultNavBarTitleColor
    ...
    public var navBarBackgroundImage: UIImage?
    {
        get {
            guard let bgImage = objc_getAssociatedObject(self, &AssociatedKeys.navBarBackgroundImage) as? UIImage else {
                return GTANavigationBar.defaultNavBarBackgroundImage
            }
            return bgImage
        }
        //        set {
    ...
    public var navBarBarTintColor: UIColor {
        get {
            guard let barTintColor = objc_getAssociatedObject(self, &AssociatedKeys.navBarBarTintColor) as? UIColor else {
                return GTANavigationBar.defaultNavBarBarTintColor
            }
            return barTintColor
        }
        set {
    ...
    public var navBarTintColor: UIColor {
        get {
            guard let tintColor = objc_getAssociatedObject(self, &AssociatedKeys.navBarTintColor) as? UIColor else {
                return GTANavigationBar.defaultNavBarTintColor
            }
            return tintColor
        }
        set {
    ...
    public var navBarTitleColor: UIColor {
        get {
            guard let titleColor = objc_getAssociatedObject(self, &AssociatedKeys.navBarTitleColor) as? UIColor else {
                return GTANavigationBar.defaultNavBarTitleColor
            }
            return titleColor
        }
        set {
    ...
    public var statusBarStyle: UIStatusBarStyle {
        get {
            guard let style = objc_getAssociatedObject(self, &AssociatedKeys.statusBarStyle) as? UIStatusBarStyle else {
                return GTANavigationBar.defaultStatusBarStyle
            }
            return style
        }
        set {
    ...
    public var navBarShadowImageHidden:Bool {
        get {
            guard let isHidden = objc_getAssociatedObject(self, &AssociatedKeys.navBarShadowImageHidden) as? Bool else {
                return GTANavigationBar.defaultShadowImageHidden
            }
            return isHidden
        }
        set {
    ...
    public func canUpdateNavigationBar() -> Bool
    {
        ...
        let middleFrame = CGRect(x: 0, y: GTANavigationBar.navBarBottom(), width: GTANavigationBar.screenWidth(), height: GTANavigationBar.screenHeight()-GTANavigationBar.navBarBottom())
        let minFrame = CGRect(x: 0, y: GTANavigationBar.navBarBottom(), width: GTANavigationBar.screenWidth(), height: GTANavigationBar.screenHeight()-GTANavigationBar.navBarBottom()-GTANavigationBar.tabBarHeight())
        // 蝙蝠🦇
